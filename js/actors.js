...
async function filter_movie(value){
    await fetch('https://api.themoviedb.org/3/search/person?query='+value+'&include_adult=false&language=en-US&page=1', options)
    .then(response => response.json())
    .then(response =>{ insert_actors(response.results);  })
...
async function insert_actors(actors){
    ...
    for(let x=0; x<total; x++){
        ...
        imgElement.src = "https://image.tmdb.org/t/p/original"+actors[x].profile_path;
        imgElement.alt = '';
...
async function add_actors_db(){
    ...
    if(streamingsArray.length>0){
        ...
        {
            interests: {actors : streamingsArray}
